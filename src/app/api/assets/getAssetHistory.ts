...
export type GetAssetHistoryParams = {
    ...
    start?: number; // asset id, e.g. asset id
    end?: number; // UNIX time in milliseconds up to 11 years for d1, e.g. 1528470720000
};

export default async function getAssetHistory({params, id}: {params: GetAssetHistoryParams; id: string}) {
    try {
        const strUrl = setQueryParams(`${ENV_VARIABLES.COINCAP_API_URL}/assets/${id}/history`, params);
